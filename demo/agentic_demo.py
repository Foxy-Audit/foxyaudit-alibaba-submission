...
def compose_override() -> str:
    """Write the override that carries the KEK into backend and worker."""
...
def main() -> int:
    ...
    try:
        ...
        if not qwen_key:
            ctx["judge"] = None
            say("⚠ THE JUDGE IS UNAVAILABLE. No QWEN_API_KEY in the environment or "
                "in backend/.env.")
        ...
        skipped = []
        if ctx["judge"] != "qwen":
            skipped = [b for b in beats if b in NEEDS_JUDGE]
            beats = [b for b in beats if b not in NEEDS_JUDGE]
