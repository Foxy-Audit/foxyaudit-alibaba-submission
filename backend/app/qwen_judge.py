...
_PRIOR_REVIEWS_TOOL = {
    ...
    "function": {
        ...
        "description": (
            ...
            f"over the last {PRIOR_REVIEW_WINDOW_DAYS} days. ⚠ ALL-ZERO COUNTS "
            "CARRY NO INFORMATION AND MUST NOT CHANGE YOUR DECISION. Zero is "
            "the state of every tag nobody has reviewed yet — a new workspace, "
            "a new tag, a fresh deployment — so it means only that no human has "
            "looked, NOT that this is safe and NOT that it has never been "
            "escalated. If every count is zero, decide exactly as you would "
            "have if you had not asked. This lookup can lower your concern only "
            "when humans actually CLEARED escalations; it can never raise your "
            "confidence out of an absence. Returns counts only: how many were "
...
def _build_system_prompt(policy_config: dict[str, Any] | None,
    ...
    if lookup_offered:
        lookup_clause = (
            ...
            "counts zero MUST NOT CHANGE YOUR DECISION — decide as if you had "
            "not asked. Zero is the state of every tag no human has reviewed "
            "yet, so it means nobody has looked, NOT that this is safe. Only "
            "non-zero counts carry information, and only a human who CLEARED "
            "something can lower your concern. It carries no content and no "
